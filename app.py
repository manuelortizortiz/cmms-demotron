...
def ensure_schema():
    ...
    q("""
    CREATE TABLE IF NOT EXISTS plan_mantenciones (
        codigo TEXT,
        tipo_equipo TEXT,
        familia TEXT,
        control TEXT,
...
def page(title, body):
    ...
    <title>{title}</title>{CSS}</head><body>{topbar()}{body}</body></html>
    """)
...
def login():
    ...
    <!doctype html><html><head><meta charset="utf-8">{{css|safe}}</head><body class="login-body">
    <form class="login-card" method="post">
      <h1>DEMOTRON</h1><p>CMMS Operativo</p>
      <label>Usuario</label><input name="usuario" value="admin">
...
def dashboard():
    ...
    compras = q("SELECT COALESCE(SUM(costo_pm_clp),0) AS n FROM compras")[0]["n"] if table_exists("compras") and col_exists("compras","costo_pm_clp") else 0
    bodega = q("SELECT COUNT(*) AS n FROM bodega")[0]["n"] if table_exists("bodega") else 0
    ...
    crit = "".join(f"<tr><td><b>{e['codigo']}</b></td><td>{e['tipo_equipo']}</td><td>{e['ubicacion']}</td><td>{e['lectura_actual']}</td><td>{badge(e['estado'])}</td></tr>" for e in equipos if str(e["estado"]).upper() not in ["AL DÍA","AL DIA"])[:8000]

    cards = "".join(f"""
      <div class="machine-card">
        <h4>{e['codigo']}</h4>
        <div class="machine-img">{'🚚' if 'cam' in str(e['tipo_equipo']).lower() else '🚜' if 'excav' in str(e['tipo_equipo']).lower() else '🚙' if 'veh' in str(e['familia']).lower() else '⚙️'}</div>
        <p>{e['marca']} {e['modelo']}</p><p>{e['ubicacion']}</p><p>{badge(e['estado'])}</p>
      </div>
    """ for e in equipos[:30])
...
def form_input(name, label, typ="text", value=""):
    return f"<label>{label}</label><input name='{name}' type='{typ}' value='{value or ''}'>"

def build_form(action, fields):
    inputs = "".join(form_input(*f) for f in fields)
    return f"<form class='form-card' method='post' action='{action}'>{inputs}<button>Guardar</button></form>"
...
def equipos():
    ...
    if request.method == "POST":
        ...
        q("""
        INSERT INTO maestro_equipos (codigo,tipo_equipo,familia,marca,modelo,ano,ubicacion,responsable,lectura_actual,unidad,proxima_pm,estado)
        VALUES (:codigo,:tipo_equipo,:familia,:marca,:modelo,:ano,:ubicacion,:responsable,:lectura_actual,:unidad,:proxima_pm,:estado)
        ON CONFLICT (codigo) DO UPDATE SET
        tipo_equipo=EXCLUDED.tipo_equipo,familia=EXCLUDED.familia,marca=EXCLUDED.marca,modelo=EXCLUDED.modelo,
        ano=EXCLUDED.ano,ubicacion=EXCLUDED.ubicacion,responsable=EXCLUDED.responsable,lectura_actual=EXCLUDED.lectura_actual,
        unidad=EXCLUDED.unidad,proxima_pm=EXCLUDED.proxima_pm,estado=EXCLUDED.estado
        """, data, fetch=False)
        return redirect(url_for("equipos"))

    equipos = get_equipos()
    rows = "".join(f"<tr><td><b>{e['codigo']}</b></td><td>{e['tipo_equipo']}</td><td>{e['familia']}</td><td>{e['marca']}</td><td>{e['modelo']}</td><td>{e['ano']}</td><td>{e['ubicacion']}</td><td>{e['responsable']}</td><td>{e['lectura_actual']}</td><td>{e['unidad']}</td><td>{e['proxima_pm']}</td><td>{badge(e['estado'])}</td></tr>" for e in equipos)
    form = build_form("/equipos", [
        ("codigo","Código"),("tipo_equipo","Tipo de Equipo"),("familia","Familia"),("marca","Marca"),("modelo","Modelo"),
        ("ano","Año"),("ubicacion","Ubicación"),("responsable","Responsable"),("lectura_actual","Lectura Actual"),
        ("unidad","Unidad"),("proxima_pm","Próxima PM"),("estado","Estado")
    ])
    body = f"<main class='data-page'><div class='data-head'><h2>Equipos</h2><a class='btn' href='/admin/importar-cmms'>Importar CMMS</a></div>{form}<div class='table-card'><table><thead><tr><th>Código</th><th>Tipo</th><th>Familia</th><th>Marca</th><th>Modelo</th><th>Año</th><th>Ubicación</th><th>Responsable</th><th>Lectura</th><th>Unidad</th><th>Próx PM</th><th>Estado</th></tr></thead><tbody>{rows}</tbody></table></div></main>"
...
def lecturas():
    ...
    data = q("SELECT * FROM lecturas ORDER BY fecha DESC NULLS LAST LIMIT 1000")
    rows = "".join(f"<tr><td>{safe(r.get('fecha'))}</td><td><b>{safe(r.get('codigo'))}</b></td><td>{safe(r.get('horometro'))}</td><td>{safe(r.get('kilometraje'))}</td><td>{norm_ubic(r.get('obra_ubicacion'))}</td><td>{safe(r.get('responsable'))}</td><td>{safe(r.get('observacion'))}</td></tr>" for r in data)
    ...
    body = f"<main class='data-page'><h2>Lecturas</h2>{form}<div class='table-card'><table><thead><tr><th>Fecha</th><th>Código</th><th>Horómetro</th><th>Kilometraje</th><th>Ubicación</th><th>Responsable</th><th>Obs</th></tr></thead><tbody>{rows}</tbody></table></div></main>"
    return page("Lecturas", body)
...
def mantenciones():
    ...
    data = q("SELECT * FROM mantenciones ORDER BY fecha DESC NULLS LAST LIMIT 1000")
    rows = "".join(f"<tr><td>{safe(r.get('fecha'))}</td><td><b>{safe(r.get('codigo'))}</b></td><td>{safe(r.get('tipo_mantencion'))}</td><td>{safe(r.get('lectura'))}</td><td>{safe(r.get('folio'))}</td><td>{safe(r.get('proveedor'))}</td><td>{safe(r.get('costo_mantencion_clp'))}</td><td>{badge(r.get('estado'))}</td></tr>" for r in data)
    form = build_form("/mantenciones", [("fecha","Fecha","date"),("codigo","Código"),("tipo_mantencion","Tipo Mantención"),("lectura","Lectura","number"),("espm","Descripción/ESPM"),("folio","Folio/OT"),("lugar","Lugar"),("proveedor","Proveedor"),("costo_mantencion_clp","Costo","number"),("estado","Estado")])
    body = f"<main class='data-page'><h2>Mantenciones</h2>{form}<div class='table-card'><table><thead><tr><th>Fecha</th><th>Código</th><th>Tipo</th><th>Lectura</th><th>Folio/OT</th><th>Proveedor</th><th>Costo</th><th>Estado</th></tr></thead><tbody>{rows}</tbody></table></div></main>"
...
def ot():
    ...
    data = q("SELECT * FROM ot ORDER BY fecha DESC NULLS LAST LIMIT 1000")
    rows = "".join(f"<tr><td>{safe(r.get('fecha'))}</td><td><b>{safe(r.get('ot'))}</b></td><td>{safe(r.get('codigo'))}</td><td>{safe(r.get('tipo'))}</td><td>{safe(r.get('lectura'))}</td><td>{safe(r.get('descripcion'))}</td><td>{safe(r.get('responsable'))}</td><td>{badge(r.get('estado'))}</td><td>{safe(r.get('costo'))}</td></tr>" for r in data)
    form = build_form("/ot", [("fecha","Fecha","date"),("ot","OT/Folio"),("codigo","Código"),("tipo","Tipo"),("lectura","Lectura","number"),("descripcion","Descripción"),("responsable","Responsable"),("estado","Estado"),("costo","Costo","number")])
    body = f"<main class='data-page'><h2>Órdenes de Trabajo</h2>{form}<div class='table-card'><table><thead><tr><th>Fecha</th><th>OT</th><th>Código</th><th>Tipo</th><th>Lectura</th><th>Descripción</th><th>Responsable</th><th>Estado</th><th>Costo</th></tr></thead><tbody>{rows}</tbody></table></div></main>"
    return page("OT", body)
...
def compras():
    ...
    data = q("SELECT * FROM compras ORDER BY fecha DESC NULLS LAST LIMIT 1000")
    rows = "".join(f"<tr><td>{safe(r.get('fecha'))}</td><td><b>{safe(r.get('oc'))}</b></td><td>{safe(r.get('codigo'))}</td><td>{safe(r.get('descripcion'))}</td><td>{safe(r.get('proveedor'))}</td><td>{safe(r.get('costo_pm_clp'))}</td><td>{safe(r.get('regla'))}</td><td>{badge(r.get('estado_oc'))}</td></tr>" for r in data)
    form = build_form("/compras", [("fecha","Fecha","date"),("oc","OC"),("codigo","Código"),("descripcion","Descripción"),("proveedor","Proveedor"),("costo_pm_clp","Monto","number"),("regla","Regla"),("estado_oc","Estado OC")])
    body = f"<main class='data-page'><h2>Compras PM</h2>{form}<div class='table-card'><table><thead><tr><th>Fecha</th><th>OC</th><th>Código</th><th>Descripción</th><th>Proveedor</th><th>Monto</th><th>Regla</th><th>Estado</th></tr></thead><tbody>{rows}</tbody></table></div></main>"
    return page("Compras", body)
...
def bodega():
    ...
    data = q("SELECT * FROM bodega ORDER BY fecha DESC NULLS LAST LIMIT 1000")
    rows = "".join(f"<tr><td><b>{safe(r.get('folio'))}</b></td><td>{safe(r.get('fecha'))}</td><td>{safe(r.get('equipo'))}</td><td>{safe(r.get('envio'))}</td><td>{safe(r.get('persona_que_retiro'))}</td><td>{safe(r.get('destino'))}</td><td>{safe(r.get('comentario'))}</td><td>{safe(r.get('codigo'))}</td></tr>" for r in data)
    ...
    body = f"<main class='data-page'><h2>Bodega</h2>{form}<div class='table-card'><table><thead><tr><th>Folio</th><th>Fecha</th><th>Equipo</th><th>Envío</th><th>Retira</th><th>Destino</th><th>Comentario</th><th>Código</th></tr></thead><tbody>{rows}</tbody></table></div></main>"
    return page("Bodega", body)
...
def proyeccion():
    ...
    rows = "".join(f"<tr><td><b>{safe(r.get('codigo'))}</b></td><td>{safe(r.get('tipo_equipo'))}</td><td>{safe(r.get('control'))}</td><td>{safe(r.get('lectura_actual'))}</td><td>{safe(r.get('proxima_lectura_objetivo'))}</td><td>{safe(r.get('promedio_diario'))}</td><td>{safe(r.get('dias_estimados'))}</td><td>{safe(r.get('fecha_estimada'))}</td><td>{badge(r.get('estado_operativo'))}</td><td>{safe(r.get('prioridad'))}</td><td>{safe(r.get('accion_sugerida'))}</td></tr>" for r in data)
    body = f"<main class='data-page'><h2>Proyección PM</h2><div class='table-card'><table><thead><tr><th>Código</th><th>Tipo</th><th>Control</th><th>Lectura</th><th>Próxima</th><th>Promedio</th><th>Días</th><th>Fecha</th><th>Estado</th><th>Prioridad</th><th>Acción</th></tr></thead><tbody>{rows}</tbody></table></div></main>"
