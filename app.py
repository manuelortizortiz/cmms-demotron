...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "demotron-railway-postgres")
...
def init_db():
    with engine.begin() as conn:
        ...
        CREATE TABLE IF NOT EXISTS ot (
            ...
            fecha_cierre TEXT,
            lectura_cierre DOUBLE PRECISION DEFAULT 0,
...
def load_main():
    ...
    def choose_values(row):
        pm_candidates = []
        if row["ultima_lectura_pm"] > 0: pm_candidates.append(row["ultima_lectura_pm"])
        if row["ultima_pm"] > 0: pm_candidates.append(row["ultima_pm"])
        pm_real = max(pm_candidates) if pm_candidates else 0
        candidates = []
        if row["lectura_actual_excel"] > 0: candidates.append(("Lectura Actual", row["lectura_actual_excel"]))
        if row["ultimo_horometro"] > 0: candidates.append(("Último Horómetro", row["ultimo_horometro"]))
        if row["ultimo_kilometraje"] > 0: candidates.append(("Último Kilometraje", row["ultimo_kilometraje"]))
    ...
    def estado(row):
        ...
        if "FUERA" in op or "NO OPER" in op: return "FUERA DE SERVICIO"
        if "TALLER" in op: return "EN TALLER"
        if "PROCESO" in excel: return "EN PROCESO"
        if "POR RECIBIR" in excel: return "POR RECIBIR"
        if "PENDIENTE" in excel: return "PENDIENTE"
        if row["ultima_pm_real"] <= 0: return "SIN HISTORIAL PM"
        if row["lectura_real"] <= 0: return "SIN LECTURA"
        if row["margen_real"] < 0: return "ATRASADA"
        limite = max(15, row["frecuencia_base"] * 0.10)
        if row["margen_real"] <= limite: return "PRÓXIMA"
        return "AL DÍA"

    def color(e):
        e = str(e).upper()
        if "ATRAS" in e: return "red"
        if "SIN HISTORIAL" in e or "SIN LECTURA" in e: return "orange"
        if "PRÓXIMA" in e or "PROXIMA" in e: return "yellow"
        if "PROCESO" in e or "POR RECIBIR" in e or "PENDIENTE" in e: return "blue"
        if "TALLER" in e or "FUERA" in e: return "gray"
        return "green"

    def accion(row):
        e = row["estado_calculado"]
        if e == "ATRASADA": return "Crear OT preventiva inmediata y validar repuestos."
        if e == "SIN HISTORIAL PM": return "Regularizar historial PM."
        if e == "SIN LECTURA": return "Solicitar lectura real de terreno."
        if e == "PRÓXIMA": return "Planificar compra, bodega y ventana."
        if e in ["EN PROCESO", "POR RECIBIR", "PENDIENTE"]: return "Seguimiento hasta cierre técnico."
        if e in ["EN TALLER", "FUERA DE SERVICIO"]: return "Controlar retorno operacional."
        return "Monitorear."
...
def get_ots(): return query_rows("SELECT * FROM ot ORDER BY id DESC")
def get_mantenciones(): return query_rows("SELECT * FROM mantenciones ORDER BY id DESC LIMIT 300")
def get_bodega(): return query_rows("SELECT * FROM bodega ORDER BY id DESC LIMIT 300")
def get_lecturas(): return query_rows("SELECT * FROM lecturas ORDER BY id DESC LIMIT 500")
def get_compras(): return query_rows("SELECT * FROM compras ORDER BY id DESC LIMIT 500")
def get_ubicaciones(): return query_rows("SELECT * FROM ubicaciones ORDER BY id DESC LIMIT 500")
...
def build_kpis(df, ots, bodega, compras, lecturas):
    ...
    if not df.empty:
        ...
        out.update({
            "total": int(len(df)), "controlado": round(((al_dia + proceso + proximos) / base_count * 100), 1) if base_count else 0,
            "atrasados": int(atrasados), "proximos": int(proximos), "proceso": int(proceso), "sin_historial": int(sin_hist),
            "no_operativos": int(no_oper), "costo_total": int(df["costo_total_pm"].sum())
        })
    return out
...
def crear_ot():
    ...
    with engine.begin() as conn:
        ...
        """), dict(
            numero=numero, codigo=request.form.get("codigo","").strip(), tipo=request.form.get("tipo","Preventiva"),
            prioridad=request.form.get("prioridad","Media"), estado="ABIERTA", fecha=fecha,
            lectura=float(request.form.get("lectura") or 0), descripcion=request.form.get("descripcion","").strip(),
            responsable=request.form.get("responsable","").strip(), costo=float(request.form.get("costo_estimado") or 0)
        ))
...
def cerrar_ot(ot_id):
    with engine.begin() as conn:
        ...
        conn.execute(text("""
            UPDATE ot SET estado='CERRADA', fecha_cierre=:fecha, lectura_cierre=:lectura, comentario_cierre=:comentario
            WHERE id=:id
        """), {"fecha": fecha, "lectura": lectura, "comentario": comentario, "id": ot_id})
        conn.execute(text("""
            INSERT INTO mantenciones(ot_numero,codigo,fecha,tipo,lectura,costo,descripcion,responsable)
...
def registrar_lectura():
    ...
    with engine.begin() as conn:
        ...
        if ubicacion:
            conn.execute(text("""
                INSERT INTO ubicaciones(fecha,codigo,ubicacion,responsable,observacion)
