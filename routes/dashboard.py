...
def dashboard():
    try:
        ...
        # MTTR (Tiempo Medio de Reparación) - Estimado en horas
        mttr_horas = 0
        ...
        # MTBF (Tiempo Medio Entre Fallas) - Estimado en días operativos vs fallas
        dias_operacion_total = total_eq * 30 # Asumiendo un mes de análisis baseline
        mtbf_dias = round(dias_operacion_total / len(correctivas), 1) if len(correctivas) > 0 else dias_operacion_total

        # 3. CONSTRUCCIÓN DE DATASETS PARA EL FRONTEND
        marcas_stats = {}
        equipos_dict = []
        eventos_calendario = []
        scatter_riesgo = []
        ...
        for e in eqs_db:
            ...
            # Matriz de Riesgo (Disponibilidad estimada vs Costo)
            disp_eq = 100 if e.estado_base == 'Operativo' else (50 if e.estado_base == 'Taller' else 0)
            if c_eq > 0 or f_eq > 0:
                scatter_riesgo.append({
                    'x': c_eq, 'y': disp_eq, 'r': max(5, f_eq * 3), 'codigo': e.codigo
                })
                ...
                if margen >= 0 and e.estado_base != 'Fuera de Servicio':
                    ...
                    if dias_est <= 30:
                        eventos_calendario.append({
                            'title': f"PM: {e.codigo}",
                            'start': fecha_iso,
                            'backgroundColor': '#DBEAFE', # Azul suave (Estilo Mirro)
                            'textColor': '#1E3A8A',
        ...
        return render_template('index.html', kpis=kpis, eqs=equipos_dict, mants_prev=preventivas, 
                               mants_corr=correctivas, compras=compras_db, bodega=bodega_db, 
